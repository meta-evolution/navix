"""Example of using ES-MLP on CIFAR-100 with the NGT package alias."""
...
from ngt import (
    ...
    train_step, train_step_with_fitness, evaluate, load_cifar100
)
import numpy as np
...
def main():
    ...
    parser = argparse.ArgumentParser(description='CIFAR-100 ES-MLP Classifier using NGT package')
    ...
    x_train, y_train, x_test, y_test = load_cifar100(data_dir="/root/workspace/Datasets/cifar-100-python")
    
    # ----- Model Initialization -----
    print(f"\nInitializing model: input={x_train.shape[1]}, hidden={args.hidden}, output=100")
    
    # Create model with 3 layers: input -> hidden -> hidden -> output (100 classes for CIFAR-100)
    layer_sizes = [x_train.shape[1], args.hidden, args.hidden, 100]
    ...
    filename = f"es_mlp_cifar100_h{args.hidden}_g{args.gen}_p{args.pop}_results.png"
    ...
    data_filename = f"es_mlp_cifar100_h{args.hidden}_g{args.gen}_p{args.pop}_data.txt"
    data_filepath = results_dir / data_filename
    
    with open(data_filepath, 'w') as f:
        f.write("# ES-MLP CIFAR-100 Training Results\n")
    ...
    # 保存直方图张量和相关元数据
    histogram_file = results_dir / f"fitness_histograms_cifar100_{args.hidden}h_{args.gen}gen_{args.pop}pop.npz"
