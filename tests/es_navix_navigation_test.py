...
def initialize_batch_environments(env, popsize, base_key):
    """Initialize batch of environments in parallel with fixed goal position."""
    env_keys = jax.random.split(base_key, popsize)
    timesteps = jax.vmap(env.reset)(env_keys)

    # Set goal position 3 pixels ahead in agent's initial direction
    def set_goal(ts):
        player = ts.state.entities['player']
        pos = player.position.squeeze()
        dir_ = player.direction.squeeze()
        ...
        goal_pos = jnp.clip(goal_pos, 0, jnp.array([15, 15]))  # Assuming 16x16 grid
        new_goal = ts.state.entities['goal'].replace(position=goal_pos[None, :])
        new_entities = {**ts.state.entities, 'goal': new_goal}
        return ts.replace(state=ts.state.replace(entities=new_entities))

    return jax.vmap(set_goal)(timesteps)
...
def evaluate_population_fitness(env, agent, batch_timesteps, max_steps=500, generation=None, seed=42):
    ...
    for step in range(max_steps):
        ...
        if viz_env_id is not None:
            viz_goals_reached = goals_reached[viz_env_id]
            print(f"Goals reached: {viz_goals_reached}")
